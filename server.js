...
const BUILTIN_SHOP_PRICES = Object.freeze({
  fox: 675, dragon: 2700, ninja: 1800, skull: 2025, polarbear: 1013,
  lion: 1913, croc: 900, frog: 788, phoenix: 3375, robot: 2250,
  panda: 1125, shark: 2138, rabbit: 900, penguin: 450, octopus: 2025,
  yeti: 1913, kiz_ates: 3150, kiz_buz: 3150, kiz_samurai: 3600,
  kiz_peri: 4050, kiz_neon: 4500, kiz_vampir: 4950, kiz_orman: 3375,
  kiz_deniz: 3600, kiz_sakura: 3825, kiz_karanlik: 5400,
  skin_aurora: 220, skin_storm: 220, skin_magma: 220, skin_void: 220,
  skin_solar: 220, skin_moon: 220, skin_emerald: 220, skin_sapphire: 220,
...
async function handleApi(request, response, requestPath) {
  ...
  if (requestPath === '/api/diamonds/buy' && request.method === 'POST') {
    ...
    const totalCost = amount * 100000;
    if ((user.coins || 0) < totalCost) {
      sendJson(response, 400, { error: `Yetersiz coin. ${amount} diamond için ${totalCost.toLocaleString('tr-TR')} coin gerekli.` });
      return true;
    }
...
function applySpikeDamageToMob(mob, spike, attacker, now = Date.now()) {
  ...
  if (mob.hp <= 0) {
    ...
    if (attackerSocket) {
      ...
      attackerSocket.emit('mob_kill_reward', { xp: rewardXp, gold: rewardGold, score: rewardScore, typeName: mob.typeName });
    }
